...
for c in classes:
    ...
    train_features = np.zeros(
        (n_train_samples, n_mfcc))  # we initialize the metrics. i'm going to use this feature for the machine.
    for index, f in enumerate(class_train_files):
        audio, fs = librosa.load(os.path.join(train_root, f),
                                 sr=None)  # first is the directory, the second f is the file
        mfcc = compute_mfcc(audio, fs, n_mfcc)  # now I've a matrix of features.
        train_features[index, :] = np.mean(mfcc, axis=1)
        print(f)

    dict_train_features[
        c] = train_features  # now we use a dictionary structure to save all features (is a sort of struct)
